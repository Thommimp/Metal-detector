...
{
 // Configure SDA (PC4) and SCL (PC5) as inputs with pull-up resistors
    DDRC &= ~((1 << PC4) | (1 << PC5));  // Set PC4 (SDA) and PC5 (SCL) as inputs
    PORTC |= (1 << PC4) | (1 << PC5);   // Enable pull-up resistors on PC4 and PC5
	 _delay_ms(1000);
	TWBR=18;
	TWSR&=0xFC;
	TWCR=0x05;
